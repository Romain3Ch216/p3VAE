#The following code wa taken from https://github.com/ElementAI/baal released under
#the following Apache License 2.0:
#===============================================================================
#                               Apache License
...
class MultiViewDropout(_DropoutNd):
    """
    """

    def __init__(self, img):
        super().__init__(p=0, inplace=False)
        self.compute_views(img)

    def forward(self, x):
        view_id = np.random.randint(len(self.views))
        mask = self.views[view_id]
        return torch.mul(x, mask)

    def compute_views(self, img, n_views=4, min_width=30):
        ...
        * Output:
        ...
        self.view_indices = [0] + list(self.view_indices) + [img.shape[-1]]
        for i in range(len(self.view_indices)-1):
            view = torch.zeros(img.shape[-1])
            view[self.view_indices[i]:self.view_indices[i+1]] = 1
            self.views[i] = view
